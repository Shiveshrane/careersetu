...
const historyForAPI = activeSession 
    ? activeSession.messages
        .filter(m => m.content !== 'Setting up your lesson…') 
...
case 'graph_done': {
  ...
  break;
            ...
            if (ev.history && Array.isArray(ev.history)) {
    setActiveSession(prev => {
      if (!prev) return null;
      
      const updatedMessages = ev.history
        .filter((m: any) => {
          if (m.role === 'system') return false;
          // Filter out internal JSON actions so they don't appear in chat bubbles
          try {
        ...
        .map((m: any) => ({
          id: generateId(),
          role: m.role,
          // FIX: Force content to be a string to prevent React child error
      ...
      return { ...prev, messages: updatedMessages };
            ...
            // We don't break the loop here because audio_done 
            // might still be coming in the same buffer
            break;
          }
  ...
  lastCardIdRef.currentRef.current = null;
  }, []);
